        ...
        public static async Task<List<TuringPanelDevice>> GetSerialDevices()
        {
            ...
            try
            {
                ...
                return await Task.Run(() =>
                {
                    ...
                    // Check for CT13INCH panel (VID_1A86&PID_CA11)
                    // When present, the companion 0525:A4A7 port is NOT the display —
                    // communication happens via the 1A86:CA11 port instead
                    ...
                    if (hasCt13Inch)
                    {
                        Logger.Information("Detected CT13INCH panel — using 1A86:CA11 port for communication");
                    }
                    ...
                    foreach (ManagementObject queryObj in serialPorts)
                    {
                        ...
                        // When CT13INCH is present, skip the companion 0525:A4A7 port
                        // (it's the SoC serial interface, not the display communication port)
                        ...
                        foreach (var kv in TuringPanelModelDatabase.Models)
                        {
                            ...
                            {
                                var modelInfo = TuringPanelModelDatabase.Models[kv.Key];
